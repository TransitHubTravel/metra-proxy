...
app.get("/gtfs/:feed", async (req, res) => {
  ...
  if (!jsonFeeds.includes(feed)) return res.status(400).json({ error: "Invalid JSON feed requested." });

  try {
    const response = await fetch(`${JSON_FEEDS_BASE}/${feed}`, {
      headers: {
        Authorization: "Basic " + Buffer.from(`${METRA_USERNAME}:${METRA_PASSWORD}`).toString("base64")
      }
    });

    if (!response.ok) return res.status(response.status).json({ error: "Failed to fetch JSON feed." });
...
app.get("/gtfs/raw/:file", async (req, res) => {
  ...
  if (!rawFeeds.includes(file)) return res.status(400).json({ error: "Invalid raw feed file requested." });
  ...
  try {
    ...
    if (!response.ok) return res.status(response.status).json({ error: "Failed to fetch raw file." });
